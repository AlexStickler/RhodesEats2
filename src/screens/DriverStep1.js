...
class DriverStep1 extends Component {
  constructor(props) {
    super(props);
    ...
    this.driverRef = this.getRef().child('drivers');
  }
  ...
  getItems(itemsRef){
    ...
    itemsRef.on('value', (snap) => {
      ...
      snap.forEach((child) => {
        if (child.val().status == ''){ 
          //if (child.val().request == this.props.navigation.state.params.username){ 
        items.push({
          name: child.val().name,
...
renderRow = (item) =>{
  ...
  updateStatus = (item) =>{
    ...
    this.itemsRef.child(item._key).update({status: 'processing'});
    this.itemsRef.child(item._key).update({driver: this.props.navigation.state.params.username});
   ...
   const username = this.props.navigation.state.params.username;
    return (
      ...
      <Text style={styles.text}>
       ...
       <Text style={styles.text}>
        ...
        this.updateStatus(this.state.item); 
          if (this.state.item.status == 'processing'){
          Alert.alert(
      ...
      />
        ...
        <AddButton onPress={() => this.props.navigation.navigate('DriverStep2', {username: username})} title="View Delivery List" />
      </View>
    );
  }
}
